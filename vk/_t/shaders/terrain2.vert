...
void main() {
  ...
  vec2 pShifted = position.xy - vec2(0.5);
  vec2 pMorphed = pShifted * addTileMorph * 2.0;

  float morph = max(step(0.0, pMorphed.x) * pMorphed.x, step(0.0, pMorphed.y) * pMorphed.y);

  gridSpacing *= 2.0;
  vec3 posNext = floor(pos / gridSpacing) * gridSpacing;
  pos = mix(pos, posNext, morph);

  uVu = (pos.xy / terrainDims + 1.0) * 0.5;

  if (addTileScale <= 256.0) {
    fixId1 = 18.0;
    vec3 tmp1 = vec3(0.0, 0.0, 1.0);
    float len1 = 0.1;

    for (int i = 0; i < 18; i++) {
      if (all(lessThan(segments[i], vec4(0.0001)))) continue;

      vec3 tmp1_1 = sdBezier(segments[i].xy, segmentsC[i].xy, segments[i].zw, uVu);

      if (tmp1.z > tmp1_1.z) {
        fixId1 = (tmp1.z - tmp1_1.z < 0.000003) ? 0.0 : float(i);
        len1 = segmentsC[i].z;
        tmp1 = tmp1_1;
      }
    }
    ...
    mask1.xy = tmp1.xy;
    mask1.z = clamp(pow(1.0 / (tmp1.z * 80000.0), 8.0), 0.0, 1.0);
    mask1.y *= len1;
  }

  vec2 tHeightSize = vec2(128.0);
  ...
  mask1.z = min(mask1.z, clamp((1.0 - gl_Position.z / gl_Position.w) * 100.0, 0.0, 1.0));
}
